(function($,window,undefined){"use strict";var pluginName='pep';var defaults={initiate:function(){},start:function(){},drag:function(){},stop:function(){},easing:null,rest:function(){},moveTo:!1,callIfNotStarted:['stop','rest'],startThreshold:[0,0],grid:[1,1],debug:!1,activeClass:'pep-active',multiplier:1,velocityMultiplier:2.5,shouldPreventDefault:!0,allowDragEventPropagation:!0,stopEvents:'',hardwareAccelerate:!0,useCSSTranslation:!0,disableSelect:!0,cssEaseString:"cubic-bezier(0.190, 1.000, 0.220, 1.000)",cssEaseDuration:1000,shouldEase:!0,droppable:!1,droppableActiveClass:'pep-dpa',overlapFunction:!1,constrainTo:!1,removeMargins:!0,place:!0,deferPlacement:!1,axis:null,forceNonCSS3Movement:!1,elementsWithInteraction:'input',revert:!1,revertAfter:'stop',revertIf:function(){return!0},ignoreRightClick:!0};function Pep(el,options){this.name=pluginName;this.el=el;this.$el=$(el);this.options=$.extend({},defaults,options);this.$document=$(this.$el[0].ownerDocument);this.$body=this.$document.find('body');this.moveTrigger="MSPointerMove touchmove mousemove";this.startTrigger="MSPointerDown touchstart mousedown";this.stopTrigger="MSPointerUp touchend mouseup";this.startTriggerArray=this.startTrigger.split(' ');this.moveTriggerArray=this.moveTrigger.split(' ');this.stopTriggerArray=this.stopTrigger.split(' ');this.stopEvents=[this.stopTrigger,this.options.stopEvents].join(' ');if(this.options.constrainTo==='window')
    ...
    this.applyMSDefaults();this.CSSEaseHash=this.getCSSEaseHash();this.scale=1;this.started=!1;this.disabled=!1;this.activeDropRegions=[];this.resetVelocityQueue();this.init();return this}
    Pep.prototype.init=function(){if(this.options.debug)
        ...
        var initialDx=Math.abs(this.startX-curX);var initialDy=Math.abs(this.startY-curY);if(!this.started&&(initialDx>this.options.startThreshold[0]||initialDy>this.options.startThreshold[1])){this.started=!0;this.$el.addClass('pep-start');this.options.start.call(this,this.startEvent,this)}
        ...
        return;this.log({type:'event',event:ev.type});this.active=!1;this.easing=!0;this.$el.removeClass('pep-start').addClass('pep-ease');if(this.options.droppable){this.calculateActiveDropRegions()}
        ...
        var self=this;this.restTimeout=setTimeout(function(){if(self.options.droppable){self.calculateActiveDropRegions()}
            ...
            self.removeActiveClass()},this.options.cssEaseDuration)};Pep.prototype.normalizeEvent=function(ev){ev.pep={};if(this.isPointerEventCompatible()||!this.isTouch(ev)){if(ev.pageX){ev.pep.x=ev.pageX;ev.pep.y=ev.pageY}else{ev.pep.x=ev.originalEvent.pageX;ev.pep.y=ev.originalEvent.pageY}
        ev.pep.type=ev.type}
    else{ev.pep.x=ev.originalEvent.touches[0].pageX;ev.pep.y=ev.originalEvent.touches[0].pageY;ev.pep.type=ev.type}
        ...
        this.offset.left=this.$el.css('left');if(parseInt(this.$el.css('top'),10))
        this.offset.top=this.$el.css('top');if(this.options.removeMargins)
    ...
    else{this.disabled=!on}};$.extend($.easing,{easeOutQuad:function(x,t,b,c,d){return-c*(t/=d)*(t-2)+b},easeOutCirc:function(x,t,b,c,d){return c*Math.sqrt(1-(t=t/d-1)*t)+b},easeOutExpo:function(x,t,b,c,d){return(t===d)?b+c:c*(-Math.pow(2,-10*t/d)+1)+b}});$.fn[pluginName]=function(options){return this.each(function(){if(!$.data(this,'plugin_'+pluginName)){var pepObj=new Pep(this,options);$.data(this,'plugin_'+pluginName,pepObj);$.pep.peps.push(pepObj)}})};$.pep={};$.pep.peps=[];$.pep.toggleAll=function(on){$.each(this.peps,function(index,pepObj){pepObj.toggle(on)})};$.pep.unbind=function($obj){var pep=$obj.data('plugin_'+pluginName);if(typeof pep==='undefined')
        return;pep.toggle(!1);pep.unsubscribe();$obj.removeData('plugin_'+pluginName)}}(jQuery,window))